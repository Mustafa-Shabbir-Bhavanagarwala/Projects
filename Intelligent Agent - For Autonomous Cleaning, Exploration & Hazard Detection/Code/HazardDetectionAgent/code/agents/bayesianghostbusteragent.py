...
class BayesianGhostBusterAgent(VacuumAgent):
   ...
   # If there is nowhere promising to bust, return None
    def suckyShouldBust(self):
        # Your code here
        sorted_co = self.world_model.sorted_coords()
        for coord, prob in sorted_co:
            if prob > 0.5:
                return coord
        return None
    ...
    def bust_strategy(self):
        ...
        unbusted = [(coord, prob) for coord, prob in sorted_coords if coord not in busted_coords or not busted_coords[coord]]
        # Define the variable unprobed -- the list of unprobed coordinates, sorted in descending order of probability 
        unprobed = [(coord, prob) for coord, prob in sorted_coords if coord not in probed_coords]
