...
class CommandProcessor:    
            ...
            for i, word in enumerate(words):
                if is_coord(word):
                    if coord_from is None:
                        coord_from = to_coord(word)
                    else:
                        coord_to = to_coord(word)
    
        # Return a move intent with source and destination coordinates
            if coord_from and coord_to:
                return 'move_object', coord_from, coord_to
    ...
    def interpret_command(self, cmd):
        ...
        elif intent == 'suck_some_dirt':
            return intent_suck_some_dirt(self)
        elif intent == 'get_all_gold':
            return intent_get_all_gold(self)
        elif intent == 'move_dirt':
            from_coord, to_coord = parse_move_coords(cmd)
            return intent_move_dirt(self, from_coord, to_coord)
        elif intent == 'move_gold':
            from_coord, to_coord = parse_move_coords(cmd)
            return intent_move_gold(self, from_coord, to_coord)
